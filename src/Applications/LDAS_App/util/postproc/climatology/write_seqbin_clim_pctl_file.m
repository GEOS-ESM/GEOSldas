...
% write "fortran sequential" tile files with climatology info 
...
if (N_grid >= 1)
  ...
  if (strcmp(file_type,'cli') )
      ...
      % fortran_tag = N_field*4;  %angles for which the output fields will be repeated below
      %
      % count = fwrite( ifp, fortran_tag,    int_precision );
      % count = fwrite( ifp, squeeze(fieldno(:)), int_precision );
      % count = fwrite( ifp, fortran_tag,    int_precision );
