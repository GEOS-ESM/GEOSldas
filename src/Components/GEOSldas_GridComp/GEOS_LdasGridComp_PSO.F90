...
! Retrieve the coordinates so we can set them
  ...
  subroutine Run(gc, import, export, clock, rc)
    ...
    !are we running a PSO optimization here?
    pso_choice = 0
    !write(*,*) 'before pso_choice in run'
    if (pso_choice == 1) then
       !write(*,*) 'inside pso_choice in run'
       open(unit=8,file='position_vals.csv',IOSTAT=reason)
