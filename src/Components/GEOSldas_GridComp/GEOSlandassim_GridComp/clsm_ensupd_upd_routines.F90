...
contains
  ...
  subroutine get_obs_pred(                                       &
    ...
    if (get_asnow_lH)  asnow_lH = asnow_l !jpark50
    ...
    if (get_asnow_lH)       deallocate(asnow_lH) !jpark50
  ...
  subroutine get_obs_pred_comm_helper(                                           &
    ...
    real, dimension(N_cat,         N_ens), intent(inout), optional :: tsurf, FT, asnow, stemp !jpark50
    ...
    if ((opt==1) .or. (opt==2)) then
       ...
       if ( ((get_sfmc ) .and. (.not. present(sfmc )))    .or.            &
            ...
            ((get_asnow) .and. (.not. present(asnow)))    .or.            & !jpark50
  ...
  subroutine cat_enkf_increments(                               &
    ...
    ! jpark50, 28 Jul 2020 - added metforce at the subroutine for 1D snow assimilation
    ! --------------------------------------------------------------
   ...
   ! real, parameter:: wemin = 13. ! kg/m^3
    ...
    do kk=1,N_catd
     ...
     do kk = 1, N_catd
       do n_e =1, N_ens
          cat_progn_incr(kk,n_e)%tc1       = 0.0
          cat_progn_incr(kk,n_e)%tc2       = 0.0
          cat_progn_incr(kk,n_e)%tc4       = 0.0
          cat_progn_incr(kk,n_e)%qa1       = 0.0
          cat_progn_incr(kk,n_e)%qa2       = 0.0
          cat_progn_incr(kk,n_e)%qa4       = 0.0
          cat_progn_incr(kk,n_e)%capac     = 0.0
          cat_progn_incr(kk,n_e)%catdef    = 0.0
          cat_progn_incr(kk,n_e)%rzexc     = 0.0
          cat_progn_incr(kk,n_e)%srfexc    = 0.0
          do j=1,N_gt
             cat_progn_incr(kk,n_e)%ght(j) = 0.0
          end do
          do i =1, N_snow
