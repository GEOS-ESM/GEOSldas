...
contains
  ...
  subroutine GEOSldas_get_pert(                                &
    ...
    real, dimension(pert_grid_l%N_lon, pert_grid_l%N_lat, N_pert), intent(inout) :: Pert_ntrmdt
    ...
    real, dimension(pert_grid_l%N_lon, pert_grid_l%N_lat, N_pert), intent(  out) :: Pert
    ...
    character(len=*), parameter :: Iam = 'GEOSldas_get_pert'
    ...
    if (init_rseed) call init_randseed(Pert_rseed(:))
    ...
    !write (logunit,*) 'get_pert(): contradictory optional inputs. STOPPING.'
    !   stop
    !   
    !end if
    ...
    ! pchakrab: adjust_mean is commented out since we won't have
    ! access to other members at this stage
    ! do m=1,N_pert
          ...
          do j=1,pert_grid_l%N_lat
             ...
             do mm=1,N_pert
                ...
                tmp_grid(i,j) = tmp_grid(i,j) + & 
                     pert_param(m)%ccorr(mm,i,j) * Pert_ntrmdt(i,j,mm)
  ...
  subroutine GEOSldas_get_init_Pert_rseed( ens_id, init_Pert_rseed )
    
    ! get initial random seed "init_Pert_rseed" for initializing 
    ! Pert_rseed within get_pert()
    !
    ! A different random seed is necessary for each ensemble member.
    ! A different random seed is generated at different run time
    ! In this example, ens_id is meant to be a nonnegative small integer.
    ...
    integer, parameter :: RSEED_CONST0 =     -1
    !integer :: RSEED_CONST2  =    -10    ! must be negative
    integer :: clock
    ! --------------------------------------------------
    !
    ! local variables
    ...
    character(len=*), parameter :: Iam = 'GEOSldas_get_init_Pert_rseed'
    character(len=400) :: err_msg
    
    ! -------------------------------------------------------------
    call system_clock(count=clock)

    init_Pert_rseed = RSEED_CONST0 - ens_id*mod(clock,97)
  ...
  end subroutine GEOSldas_get_init_Pert_rseed
  
  subroutine GEOSldas_propagate_pert(                       &
       N_pert, pert_grid_l, pert_grid_f, dtstep,   &
       Pert_rseed,                                 &
       pert_param,                                 &
       Pert_ntrmdt,                                &
       initialize              )
    ...
    do m=1,N_pert
       ...
       if (white_in_space) then 
          call rf%generate_white_field(Pert_rseed(:), ptr2rfield)
       ...
       if (white_in_time) then
          Pert_ntrmdt(:,:,m) = rfield(xstart:xend, ystart:yend)
       else
          Pert_ntrmdt(:,:,m) = cc*Pert_ntrmdt(:,:,m) + dd*rfield(xstart:xend, ystart:yend)
       end if
