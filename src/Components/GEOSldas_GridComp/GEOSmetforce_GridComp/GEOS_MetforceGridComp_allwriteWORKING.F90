...
module GEOS_MetforceGridCompMod
  ...
  use LDAS_Point_ForceMod, only: get_forcing_point 
...
!
  ...
  subroutine Initialize(gc, import, export, clock, rc)
    ...
    ! Define whether this is point forcing case or normal cal
    ...
    elseif (point_forcing_write == 0 .and. point_forcing_read == 0)then ! if model is set to normal mode
   ...
   call MAPL_GetResource ( MAPL, AEROSOL_DEPOSITION, Label="AEROSOL_DEPOSITION:", &
    ...
    if (point_forcing_read == 0) then
        ...
        if(point_forcing_write == 1) then
          ...
          call esmf2ldas(internal%mf%TimeNxt, force_time_nxt, rc=status)
          write (YYYY,'(i4.4)') model_time_nxt%year
          ...
          !write (*,*) 'land_nt_local:'
          !write (*,*) land_nt_local
          call write_time_varying (local_id, YYYY//'_point_forcing_data.nc4', &
                                  model_time_nxt,mfDataNtp,internal%mf%zenav)
        endif
   ...
   elseif (point_forcing_read == 1) then ! If we want to read only point forcing data
       ...
       ! Get forcing data if MetForcing alarm is ringing
        ...
        !if (point_forcing_write == 1 .and. force_time_prv%month == 01 .and. force_time_prv%day == 01 &
            ...
            !
            !if (MAPL__am_I_root()) then !only need one CPU to read in the forcing data
                !call get_forcing_point(met_tag,date_time,local_id,met_force_new,model_times)
                !
                !
                !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
                !!                                                                                !
 ...
 !
        ...
        !if (ESMF_AlarmIsRinging(MetForcingAlarm)) then
          ...
          !! use something like:
          !! time_ind = FINDLOC(time,model_time_con)
          !! Tair_curr = met_force_new%Tair(:,time_ind)
          !! Tair_curr_cpu = Tair_curr(local_id)
         !
!
          !! Set exports
          !Tair = mfDataNtp%Tair
